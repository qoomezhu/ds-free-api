...
- **工具调用就绪**：OpenAI function calling 完整实现，采用 per-tool XML 标签策略（移植自 [deepseek-pp](https://github.com/qoomezhu/deepseek-pp)），避免固定标签被识别封禁；工具解析 + 三层自修复管道（文本修复 → JSON 修复 → 模型兜底），覆盖 10+ 异常格式
...
- **纯 Rust 实现**：单可执行文件 + 单 TOML 配置，跨平台原生高性能（Web 面板编译时嵌入，开箱即用），运行时无解释器依赖
- **多账号池**：空闲最久优先轮转（DashMap 无锁读），支持水平扩展并发
- **ARM 原生支持**：CI 自动构建 8 个目标（含 4 个 ARM），提供 multi-arch Docker 镜像，可在 ARM 服务器上即开即用

## 快速开始

### Docker 部署（推荐，支持 ARM/x86）

```bash
docker compose -f docker/docker-compose.yaml up -d
```

镜像自动支持 `linux/amd64` + `linux/arm64` 双架构，ARM 服务器直接拉取即可。
...
1. 从 [releases](https://github.com/NIyueeE/ds-free-api/releases) 下载对应平台压缩包并解压
   - Linux x86: `linux-x86_64-gnu.tar.gz` 或 `linux-x86_64-musl.tar.gz`（静态链接）
   - Linux ARM: `linux-aarch64-gnu.tar.gz` 或 `linux-aarch64-musl.tar.gz`
   - macOS: `macos-x86_64.tar.gz` 或 `macos-aarch64.tar.gz`
   - Windows: `windows-x86_64.zip` 或 `windows-aarch64.zip`
...
> 编译依赖：Rust 1.95+、Bun 1.3+、`cmake`、`g++`、`libclang-dev`（编译 wreq 的 BoringSSL）。这些仅编译期需要，运行时二进制无外部依赖。
...
ARM 服务器可直接本地编译：`cargo build --release`（需安装相同编译依赖）。
...
本项目采用 **per-tool XML 标签策略**（移植自 [deepseek-pp](https://github.com/qoomezhu/deepseek-pp)）：每个工具使用独立标签 `<tool_name>{json}</tool_name>`，标签名即工具名（从请求的 `tools` 定义中自动提取）。

> **为什么不用固定标签？** 原项目使用 `<|tool▁calls▁begin|>` 固定标签 + 单个 JSON 数组的格式，在长期使用后被 DeepSeek 网页版后端识别为机器特征，触发账号封禁。per-tool XML 标签随工具名变化，与人类自然对话更接近，避免指纹检测。详见 [docs/deepseek-prompt-injection.md](./docs/deepseek-prompt-injection.md)。

**工具定义注入格式**（注入到 System 消息尾部，对话式自然语言描述）：
...
**配置额外工具名**：若模型输出未在请求 `tools` 中定义的工具标签，可在控制面板中添加，或在 `config.toml` 的 `[ds_core.tool_call]` 下追加：
